...
public class ArrayKit	{
	...
	public BufferedImage rotateClockWise(BufferedImage image)	{
		BufferedImage turned = new BufferedImage(image.getHeight(), image.getWidth(), image.getType());
		for (int i = 0; i < turned.getWidth(); i++)
			for (int a = 0; a < turned.getHeight(); a++)
				turned.setRGB(i, a, image.getRGB(a, image.getHeight() - i - 1));
		return turned;
	}
	...
	public char[][]	subArray(char[][] array, int startX, int endX, int startY, int endY)	{
		char[][] sub = new char[endX-startX][endY-startY];
		for (int i = startX; i < endX; i++)
			for (int a = startY; a < endY; a++)
				sub[i-startX][a-startY] = array[i][a];
		return sub;
	}
